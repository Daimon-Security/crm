...
import { getByCreditsHistory, getById, getTransactionsByCreditId, savePaymentSurcharge, setCreditHistorySelected, setCreditSelected } from "../../redux/slices/sale-credit-slice";
import { statusCreditOptions } from "../constants/status-credit";
import useNumberFormatter from "../../redux/hooks/useNumberFormatter";
import SaleCreditPaymentsDetail from "./sale-credit-payment-detail";
...
interface SaleCreditDetailProps {
    setShowDetail: (show: boolean) => void;
    hide: boolean;
    activeTab: string
}

export const SaleCreditDetail = ({ setShowDetail, hide, activeTab }: SaleCreditDetailProps) => {
    const dispatch = useAppDispatch();
    const formatNumber = useNumberFormatter();
    const location = useLocation();
    const route = location.pathname;
    const { paymentBhavior } = useAppSelector((state: RootState) => state.clients);
    const { creditSelected, creditsHistory, transactions, paymentsDetail, isLoading } = useAppSelector((state: RootState) => state.saleCredits);
    const id = creditSelected?.id.toString();
    ...
    const rows = creditsHistory.map((credit: any, index: any) => ({
        id: index + 1,
        date: credit.date,
    ...
    async function getDetail() {
        if (id) {
            ...
            dispatch(getPaymentBhavior(creditSelected.clientId, 2, creditSelected.id));
        ...
        <>
            {creditSelected ?
                <div className={`${hide ? 'd-block' : 'd-none'}`}>
                    ...
                    <SaleCreditPaymentsDetail show={showModalPaymentDetal} onClose={() => setShowModalPaymentDetal(false)} />
...
export default SaleCreditDetail;