...
export const MyCollections = ({ collections, dateQuery, selectedCollection, type, statusCredits,
        ...
        clientNumber:
            (
                <Row className="d-flex">
                    <Col lg={3}>{collection.clientNumber}</Col>
                    <Col lg={9}>
                        <button className="btn btn-outline-success btn-sm col-12" onClick={() => handleOpenDetailCredit(collection)}>crédito</button>
                    </Col>
                </Row>
            ),
        client: collection.client,
        debtCollector: collection.debtCollector,
        concept: collection.paymentType + (collection.number? ` - Nº ${collection.number}`:''),
        paymentDate: (collection.paymentDate) ? collection.paymentDate.split('T')[0] : '-',
        typeCurrency: collection.typeCurrency.toUpperCase(),
        actualPayment: (collection.actualPayment) ? formatNumber(collection.actualPayment) : '0,00',
        action: (
            <>
                        ...
                        {collection.statusCreditHistory == '1' ?
                            <div className="d-flex col-12">
                                {collection.paymentDate ?
                                    <button className="btn btn-danger me-1 btn-sm col-lg-4 col-6 payment-button" onClick={() => handleOpenCustomMessageModal(collection, 'cancelRegisteredPayment')}>Cancelar pago</button> :
                                    <>
                                        <button className="btn btn-success btn-sm col-lg-4 col-6 payment-button" onClick={() => handleOpenCustomMessageModal(collection, 'payment')}>Pagar cuota</button>
                                        <button className="btn btn-primary btn-sm col-lg-2 col-6 ms-2 " onClick={() => handleOpenReschedulePaymentModal(collection.id, collection.paymentDueDate)}><MdDateRange /></button>
                                    </>
                                }
                                {
                                    collection.numberPayment == 1 ?
                                        <button className="btn btn-primary btn-sm ms-2 me-1 col-lg-4 col-12 payment-button" disabled={collection.paymentDate != null} onClick={() => handleOpenCustomMessageModal(collection, 'cancellationInterest')}>Pagar intereses</button>
                                        : ('')
                            ...
                            <>
                                {
                                    collection.paymentDate && collection.paymentType == 'interés' && collection.numberPayment == 1 ?
                                        <>
                                            <button className="btn btn-danger me-1 btn-sm col-lg-4 col-6 payment-button" onClick={() => handleOpenCustomMessageModal(collection, 'cancelRegisteredPaymentInterest')}>Cancelar pago</button>
                                        </> : ('')
                                }
                            </>
                        }
    ...
    function handleCloseCustomMessageModal() {
        ...
        //console.log("actualizando las cobranzas: ", dateQuery);
